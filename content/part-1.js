...
window.PART_DATA = {
  ...
  chapters: [
    {
      id: "p1-c1",
      plain: "<p>At heart, a language model is just a very good guess-the-next-word machine - like the predictive text on your phone, but far smarter. Older versions guessed by memorising which words tended to follow which. Modern ones actually learn what words <em>mean</em>, so they can handle sentences they have never seen before.</p>",
      n: 1,
      ...
      diagram2: {
        type: "figure",
        title: "Why counting runs out of road",
        svg: `<svg viewBox="0 0 360 214" role="img" aria-label="N-gram table growth versus a fixed-size embedding space">
          <text x="8" y="12" font-size="9" fill="var(--text-faint)">N-GRAM TABLE - entries needed for a 50,000-word vocabulary</text>
          <rect x="8" y="20" width="26" height="18" rx="3" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.4"/>
          <text x="44" y="33" font-size="8">bigrams   50,000² = 2.5 billion</text>
          <rect x="8" y="44" width="70" height="18" rx="3" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.4"/>
          <text x="88" y="57" font-size="8">trigrams   50,000³ = 125 trillion</text>
          <rect x="8" y="68" width="200" height="18" rx="3" fill="var(--ion-dim)" stroke="var(--ion)" stroke-width="1.6"/>
          <text x="216" y="81" font-size="8" fill="var(--ion)">4-grams   ~6×10¹⁸</text>
          <text x="8" y="104" font-size="8" fill="var(--text-faint)">…and almost every one of those cells is empty. Real text only ever</text>
          <text x="8" y="116" font-size="8" fill="var(--text-faint)">visits a vanishing fraction of them - that's the sparsity problem.</text>
          <line x1="8" y1="130" x2="352" y2="130" stroke="var(--line)" stroke-width="1.2" stroke-dasharray="4 4"/>
          <text x="8" y="148" font-size="9" fill="var(--text-faint)">EMBEDDINGS - fixed size, no matter the vocabulary</text>
          <rect x="8" y="156" width="104" height="20" rx="4" fill="var(--panel-raised)" stroke="var(--line-bright)" stroke-width="1.6"/>
          <text x="60" y="170" text-anchor="middle" font-size="8">cat  [0.8, -0.2, …]</text>
          <rect x="120" y="156" width="104" height="20" rx="4" fill="var(--panel-raised)" stroke="var(--line-bright)" stroke-width="1.6"/>
          <text x="172" y="170" text-anchor="middle" font-size="8">kitten [0.7, -0.3, …]</text>
          <rect x="232" y="156" width="112" height="20" rx="4" fill="var(--panel-raised)" stroke="var(--line-bright)" stroke-width="1.6"/>
          <text x="288" y="170" text-anchor="middle" font-size="8">rocket [-0.6, 0.9, …]</text>
          <text x="60" y="190" text-anchor="middle" font-size="8" fill="var(--ion)">↖ these two are close ↗</text>
          <text x="180" y="208" text-anchor="middle" font-size="8" fill="var(--text-faint)">so what's learned about "cat" transfers to "kitten" - a table can never do that</text>
        </svg>`,
        caption: "The table doesn't just get big, it gets emptier as it grows: each extra word of context multiplies the cells while the training text stays the same size. Embeddings sidestep this entirely - the space stays fixed, and similar words land near each other, so learning about one word carries over to its neighbours.",
      },
      math: [
        {
    ...
    {
      id: "p1-c2",
      plain: "<p>Computers only understand numbers, so the first job is turning text into numbers. Words are chopped into common chunks ('tokens'), and each chunk gets a list of numbers ('an embedding') that captures its meaning - so <em>cat</em> and <em>kitten</em> end up with almost the same numbers. It is like giving every word GPS coordinates on a map of meaning, where similar words are neighbours.</p>",
      n: 2,
    ...
    {
      id: "p1-c3",
      plain: "<p>The Transformer is the engine inside every modern AI like ChatGPT. Its key trick - <em>attention</em> - lets the model look at every other word at once and decide which ones matter for the word it is working on. It is how you glance back at <em>the trophy</em> to know what <em>it</em> refers to in 'the trophy did not fit in the case because it was too big.'</p>",
      n: 3,
    ...
    {
      id: "p1-c4",
      plain: "<p>Pretraining is how a model first learns language: it reads a huge slice of the internet and plays one endless game - cover the next word, guess it, check the answer, adjust. Do that trillions of times and it soaks up grammar, facts, and reasoning. It is like a child learning to speak from constant exposure, not from a textbook.</p>",
      n: 4,
      title: "Pretraining",
    ...
    {
      id: "p1-c7",
      plain: "<p>A trained model does not just blurt one fixed answer - at each step it builds a shortlist of likely next words and picks from it. Turn the <em>temperature</em> up and it chooses more adventurously (creative, sometimes odd); turn it down and it plays safe and predictable. It is the dial between a cautious and a free-wheeling writer.</p>",
      n: 7,
      ...
      diagram2: {
        type: "figure",
        title: "The same distribution, four decoding strategies",
        svg: `<svg viewBox="0 0 360 250" role="img" aria-label="Greedy, top-k, top-p and temperature applied to one probability distribution">
          <text x="8" y="12" font-size="9" fill="var(--text-faint)">GREEDY - always the single highest</text>
          <rect x="8" y="18" width="66" height="18" rx="4" fill="var(--ion-dim)" stroke="var(--ion)" stroke-width="1.8"/>
          <text x="41" y="31" text-anchor="middle" font-size="8">mat .42</text>
          <rect x="80" y="18" width="60" height="18" rx="4" fill="var(--panel-raised)" stroke="var(--line)" stroke-width="1" opacity="0.45"/>
          <text x="110" y="31" text-anchor="middle" font-size="8" fill="var(--text-faint)">rug .18</text>
          <rect x="146" y="18" width="66" height="18" rx="4" fill="var(--panel-raised)" stroke="var(--line)" stroke-width="1" opacity="0.45"/>
          <text x="179" y="31" text-anchor="middle" font-size="8" fill="var(--text-faint)">floor .14</text>
          <rect x="218" y="18" width="68" height="18" rx="4" fill="var(--panel-raised)" stroke="var(--line)" stroke-width="1" opacity="0.45"/>
          <text x="252" y="31" text-anchor="middle" font-size="8" fill="var(--text-faint)">couch .09</text>
          <text x="296" y="31" font-size="8" fill="var(--text-faint)">…</text>
          <text x="8" y="62" font-size="9" fill="var(--text-faint)">TOP-K (k=3) - fixed-size shortlist, then sample</text>
          <rect x="8" y="68" width="66" height="18" rx="4" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.8"/>
          <text x="41" y="81" text-anchor="middle" font-size="8">mat</text>
          <rect x="80" y="68" width="60" height="18" rx="4" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.8"/>
          <text x="110" y="81" text-anchor="middle" font-size="8">rug</text>
          <rect x="146" y="68" width="66" height="18" rx="4" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.8"/>
          <text x="179" y="81" text-anchor="middle" font-size="8">floor</text>
          <rect x="218" y="68" width="68" height="18" rx="4" fill="var(--panel-raised)" stroke="var(--line)" stroke-width="1" opacity="0.45"/>
          <text x="252" y="81" text-anchor="middle" font-size="8" fill="var(--text-faint)">cut</text>
          <text x="8" y="112" font-size="9" fill="var(--text-faint)">TOP-P (p=0.75) - shortlist grows/shrinks with confidence</text>
          <rect x="8" y="118" width="66" height="18" rx="4" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.8"/>
          <text x="41" y="131" text-anchor="middle" font-size="8">.42</text>
          <rect x="80" y="118" width="60" height="18" rx="4" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.8"/>
          <text x="110" y="131" text-anchor="middle" font-size="8">.60</text>
          <rect x="146" y="118" width="66" height="18" rx="4" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.8"/>
          <text x="179" y="131" text-anchor="middle" font-size="8">.74</text>
          <rect x="218" y="118" width="68" height="18" rx="4" fill="var(--ion-dim)" stroke="var(--ion)" stroke-width="1.8"/>
          <text x="252" y="131" text-anchor="middle" font-size="8">.83 ✓stop</text>
          <text x="292" y="131" font-size="8" fill="var(--text-faint)">cumulative</text>
          <text x="8" y="162" font-size="9" fill="var(--text-faint)">TEMPERATURE - reshapes the curve before any of the above</text>
          <text x="60" y="176" text-anchor="middle" font-size="8" fill="var(--text-faint)">T = 0.2 (sharp)</text>
          <rect x="30" y="182" width="14" height="40" fill="var(--ion)" opacity="0.85"/>
          <rect x="48" y="212" width="14" height="10" fill="var(--amber)" opacity="0.6"/>
          <rect x="66" y="216" width="14" height="6" fill="var(--amber)" opacity="0.5"/>
          <rect x="84" y="219" width="14" height="3" fill="var(--amber)" opacity="0.4"/>
          <text x="180" y="176" text-anchor="middle" font-size="8" fill="var(--text-faint)">T = 1.0</text>
          <rect x="150" y="192" width="14" height="30" fill="var(--ion)" opacity="0.85"/>
          <rect x="168" y="204" width="14" height="18" fill="var(--amber)" opacity="0.6"/>
          <rect x="186" y="209" width="14" height="13" fill="var(--amber)" opacity="0.5"/>
          <rect x="204" y="213" width="14" height="9" fill="var(--amber)" opacity="0.4"/>
          <text x="300" y="176" text-anchor="middle" font-size="8" fill="var(--text-faint)">T = 1.8 (flat)</text>
          <rect x="270" y="200" width="14" height="22" fill="var(--ion)" opacity="0.85"/>
          <rect x="288" y="203" width="14" height="19" fill="var(--amber)" opacity="0.6"/>
          <rect x="306" y="205" width="14" height="17" fill="var(--amber)" opacity="0.5"/>
          <rect x="324" y="207" width="14" height="15" fill="var(--amber)" opacity="0.4"/>
          <line x1="24" y1="222" x2="344" y2="222" stroke="var(--line-bright)" stroke-width="1.2"/>
          <text x="180" y="244" text-anchor="middle" font-size="8" fill="var(--text-faint)">low T = safe and repetitive · high T = adventurous and error-prone</text>
        </svg>`,
        caption: "Greedy always takes the top token. Top-k keeps a fixed number of candidates regardless of confidence. Top-p keeps however many are needed to reach a probability mass, so the shortlist narrows when the model is sure and widens when it isn't. Temperature is different in kind - it reshapes the distribution itself before any shortlist is drawn.",
      },
      math: [
        {
    ...
    {
      id: "p1-c5",
      plain: "<p>A freshly pretrained model knows a lot but does not yet know how to <em>behave</em> like a helpful assistant. Fine-tuning is a short apprenticeship: we show it thousands of examples of good answers so it learns the job - the way a sharp new hire picks up your team's way of doing things in their first week.</p>",
      n: 5,
      title: "Fine-Tuning",
    ...
    {
      id: "p1-c6",
      plain: "<p>Being knowledgeable is not the same as being helpful, honest, and safe. Alignment is the training that teaches a model to actually be those things - often by having people rank its answers ('this one is better than that one') and nudging it toward the preferred ones. Think coaching, not cramming: shaping judgment and manners, not adding facts.</p>",
      n: 6,
    ...
    {
      id: "p1-c8",
      plain: "<p>How do you know an AI is actually good? You test it - but that is surprisingly hard. A model can ace a benchmark because it secretly saw the answers during training, or the test can measure the wrong thing entirely. This chapter is about measuring ability honestly - the same challenge as writing a fair exam nobody can game.</p>",
      n: 8,

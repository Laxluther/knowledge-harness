...
window.PART_DATA = {
  ...
  chapters: [
    {
      id: "p2-c1",
      plain: "<p>A prompt isn't just your question - it's everything you hand the model at once: instructions, examples, and background. Writing a good one is like giving directions to a capable but very literal assistant: the clearer and more specific you are, the better the result.</p>",
      n: 1,
      ...
      diagram2: {
        type: "figure",
        title: "Anatomy of a well-built prompt",
        svg: `<svg viewBox="0 0 360 250" role="img" aria-label="The layered parts of a prompt from system role down to the request">
          <rect x="8" y="8" width="344" height="38" rx="6" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.8"/>
          <text x="18" y="24" font-size="9" fill="var(--amber)">ROLE / SYSTEM</text>
          <text x="18" y="38" font-size="9">"You are a claims analyst. Be concise and cite policy numbers."</text>
          <rect x="8" y="52" width="344" height="38" rx="6" fill="var(--panel-raised)" stroke="var(--line-bright)" stroke-width="1.6"/>
          <text x="18" y="68" font-size="9" fill="var(--text-faint)">TASK, DECOMPOSED</text>
          <text x="18" y="82" font-size="9">"1. Find the claim. 2. Check it against policy. 3. Decide."</text>
          <rect x="8" y="96" width="344" height="38" rx="6" fill="var(--panel-raised)" stroke="var(--line-bright)" stroke-width="1.6"/>
          <text x="18" y="112" font-size="9" fill="var(--text-faint)">OUTPUT FORMAT</text>
          <text x="18" y="126" font-size="9">"Reply as JSON: {verdict, policy_ref, reason}"</text>
          <rect x="8" y="140" width="344" height="52" rx="6" fill="var(--panel-raised)" stroke="var(--ion)" stroke-width="1.8"/>
          <text x="18" y="156" font-size="9" fill="var(--ion)">DELIMITED DATA - untrusted, never instructions</text>
          <text x="18" y="170" font-size="9" fill="var(--text-faint)">&lt;document&gt;</text>
          <text x="18" y="182" font-size="9" fill="var(--text-faint)">  ...the claim text being processed...  &lt;/document&gt;</text>
          <rect x="8" y="198" width="344" height="34" rx="6" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.6"/>
          <text x="18" y="214" font-size="9" fill="var(--amber)">THE REQUEST</text>
          <text x="18" y="227" font-size="9">"Assess the claim above."</text>
          <text x="180" y="246" text-anchor="middle" font-size="8" fill="var(--text-faint)">all of this is context the attention layer computes over</text>
        </svg>`,
        caption: "The question itself is the smallest part. Role, decomposed task, explicit format, and clearly delimited data do most of the work - and the delimiters are what keep the model from reading the data as new instructions.",
      },
    ...
    {
      id: "p2-c2",
      plain: "<p>You can teach a model a new task on the spot just by showing it a couple of worked examples in your message - no retraining needed. It's like showing someone two or three solved puzzles and letting them infer the pattern for the next one.</p>",
      n: 2,
    ...
    {
      id: "p2-c3",
      plain: "<p>Ask a model to <em>show its working</em> and it gets noticeably better at hard problems - because, like us, it makes fewer mistakes when it reasons step by step instead of blurting an answer. That simple nudge is the whole trick.</p>",
      n: 3,
      ...
      diagram3: {
        type: "figure",
        title: "Chain vs tree of thought",
        svg: `<svg viewBox="0 0 360 214" role="img" aria-label="A single linear chain of reasoning versus a branching tree that abandons dead ends">
          <text x="76" y="14" text-anchor="middle" font-size="9" fill="var(--text-muted)">Chain-of-thought</text>
          <rect x="40" y="26" width="72" height="22" rx="5" fill="var(--panel-raised)" stroke="var(--line-bright)" stroke-width="1.6"/>
          <text x="76" y="41" text-anchor="middle" font-size="9">step 1</text>
          <line x1="76" y1="48" x2="76" y2="66" stroke="var(--line-bright)" stroke-width="1.6"/>
          <rect x="40" y="66" width="72" height="22" rx="5" fill="var(--panel-raised)" stroke="var(--line-bright)" stroke-width="1.6"/>
          <text x="76" y="81" text-anchor="middle" font-size="9">step 2</text>
          <line x1="76" y1="88" x2="76" y2="106" stroke="var(--line-bright)" stroke-width="1.6"/>
          <rect x="40" y="106" width="72" height="22" rx="5" fill="var(--panel-raised)" stroke="var(--line-bright)" stroke-width="1.6"/>
          <text x="76" y="121" text-anchor="middle" font-size="9">step 3</text>
          <line x1="76" y1="128" x2="76" y2="146" stroke="var(--line-bright)" stroke-width="1.6"/>
          <rect x="40" y="146" width="72" height="22" rx="5" fill="var(--ion-dim)" stroke="var(--ion)" stroke-width="1.8"/>
          <text x="76" y="161" text-anchor="middle" font-size="9">answer</text>
          <text x="76" y="192" text-anchor="middle" font-size="8" fill="var(--text-faint)">one path - a wrong</text>
          <text x="76" y="203" text-anchor="middle" font-size="8" fill="var(--text-faint)">step 2 dooms the rest</text>
          <line x1="176" y1="8" x2="176" y2="206" stroke="var(--line)" stroke-width="1.2" stroke-dasharray="3 4"/>
          <text x="272" y="14" text-anchor="middle" font-size="9" fill="var(--text-muted)">Tree-of-thought</text>
          <rect x="240" y="26" width="64" height="20" rx="5" fill="var(--panel-raised)" stroke="var(--line-bright)" stroke-width="1.6"/>
          <text x="272" y="40" text-anchor="middle" font-size="9">start</text>
          <line x1="272" y1="46" x2="216" y2="66" stroke="var(--line)" stroke-width="1.4"/>
          <line x1="272" y1="46" x2="272" y2="66" stroke="var(--amber)" stroke-width="1.8"/>
          <line x1="272" y1="46" x2="328" y2="66" stroke="var(--line)" stroke-width="1.4"/>
          <rect x="190" y="66" width="52" height="20" rx="5" fill="var(--panel-raised)" stroke="var(--line)" stroke-width="1.4"/>
          <text x="216" y="80" text-anchor="middle" font-size="8" fill="var(--text-faint)">A ✕</text>
          <rect x="246" y="66" width="52" height="20" rx="5" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.8"/>
          <text x="272" y="80" text-anchor="middle" font-size="8">B ✓</text>
          <rect x="302" y="66" width="52" height="20" rx="5" fill="var(--panel-raised)" stroke="var(--line)" stroke-width="1.4"/>
          <text x="328" y="80" text-anchor="middle" font-size="8" fill="var(--text-faint)">C ✕</text>
          <line x1="272" y1="86" x2="246" y2="108" stroke="var(--amber)" stroke-width="1.8"/>
          <line x1="272" y1="86" x2="308" y2="108" stroke="var(--line)" stroke-width="1.4"/>
          <rect x="220" y="108" width="52" height="20" rx="5" fill="var(--amber-dim)" stroke="var(--amber)" stroke-width="1.8"/>
          <text x="246" y="122" text-anchor="middle" font-size="8">B1 ✓</text>
          <rect x="282" y="108" width="52" height="20" rx="5" fill="var(--panel-raised)" stroke="var(--line)" stroke-width="1.4"/>
          <text x="308" y="122" text-anchor="middle" font-size="8" fill="var(--text-faint)">B2 ✕</text>
          <line x1="246" y1="128" x2="246" y2="146" stroke="var(--ion)" stroke-width="1.8"/>
          <rect x="210" y="146" width="72" height="22" rx="5" fill="var(--ion-dim)" stroke="var(--ion)" stroke-width="1.8"/>
          <text x="246" y="161" text-anchor="middle" font-size="9">answer</text>
          <text x="272" y="192" text-anchor="middle" font-size="8" fill="var(--text-faint)">explores branches, scores</text>
          <text x="272" y="203" text-anchor="middle" font-size="8" fill="var(--text-faint)">them, abandons dead ends</text>
        </svg>`,
        caption: "Chain-of-thought commits to one line of reasoning; if an early step is wrong, everything after inherits the mistake. Tree-of-thought generates several candidate steps, evaluates them, and backtracks out of the bad ones - far more capable, and far more expensive.",
      },
    ...
    {
      id: "p2-c4",
      plain: "<p>A model can only hold so much text in mind at once - its <em>context window</em>. Your instructions, the conversation so far, and the answer all compete for that space. It's a whiteboard of fixed size: fill it with clutter and there's no room left for what matters.</p>",
      n: 4,
    ...
    {
      id: "p2-c5",
      plain: "<p>Normally a model replies in free-form sentences, which other software struggles to use. Here we make it reply in a strict format (like a filled-in form) and even let it press 'buttons' to run real code - turning a chatbot into something that can actually do things.</p>",
      n: 5,
      ...
      diagram2: {
        ...
        props: [{ id: "tool", kind: "bench", x: 84, label: "get_weather()" }],
        actors: [
          { id: "model", kind: "wizard", x: 16, label: "model" },
          { id: "drone", kind: "red", x: 30, label: "tool call" },
        ],
        steps: [
          { actor: "model", think: true, say: "The model can't know live weather - its training data is frozen." },
          { actor: "drone", to: 80, say: "So it emits a structured call: get_weather(city=\"Tokyo\")." },
          { actor: "drone", pickup: true, say: "The app runs the real function against a live weather API." },
          { actor: "drone", to: 30, say: "The result - 18°C, light rain - is carried back to the model." },
          { actor: "drone", handoff: "model", say: "…and handed back as a tool result." },
          { actor: "model", deliver: true, say: "Now it answers using real, current data instead of guessing." },
        ],
      },
